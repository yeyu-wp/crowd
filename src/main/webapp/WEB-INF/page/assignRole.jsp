...
<form:form  modelAttribute="role" action="${pageContext.request.contextPath}/user/findUserNoRole?" method="post">
			<input type="submit" class="btn btn-danger" style="float:right;margin-left:10px;" value="新增"/>					<br>
					<hr style="clear:both;">
					<div class="table-responsive">
						<table class="table  table-bordered">
							...
							<c:forEach items="${role}" var="role" varStatus="status">
								<tr>
									<td><input type="checkbox" name="selectRole" value="${role.roleId}"></td>
									...
									<td>
										...
										<button type="button" class="btn btn-danger btn-xs"  onclick="return isDel(${role.roleId})" >删除</button>
									</td>
								</tr>
							</c:forEach>
							</tbody>
...
<script type="text/javascript">
	...
	function isDel(idx) {
		...
		if (confirm("确定要删除当前角色吗？")){
			//如果是 去跳转到退出的业务
			window.location.href="${pageContext.request.contextPath}/user/delUserRole?roleId="+idx;
		}else {
			//啥也不做
		}
	}
